...
describe("audit scan", () => {
  ...
  test("skips out/, .next/, and .source/ build output directories", async () => {
    const { mkdirSync, mkdtempSync, rmSync, writeFileSync } = await import(
      "node:fs"
    );
    const { tmpdir } = await import("node:os");
    // A repo that ships an llms.txt at the root but also has Next.js + Fumadocs
    // static-export build artifacts nested deep. The audit must walk only the
    // canonical agent doc, not the generated copies.
    const dir = mkdtempSync(join(tmpdir(), "pickled-audit-ignore-"));
    try {
      writeFileSync(join(dir, "llms.txt"), "# canonical\n");
      mkdirSync(join(dir, "apps", "docs", "out"), { recursive: true });
      writeFileSync(
        join(dir, "apps", "docs", "out", "llms.txt"),
        "# generated by next build\n",
      );
      writeFileSync(
        join(dir, "apps", "docs", "out", "llms-full.txt"),
        "# generated by next build\n",
      );
      mkdirSync(join(dir, "apps", "docs", ".next"), { recursive: true });
      writeFileSync(
        join(dir, "apps", "docs", ".next", "llms.txt"),
        "# Next cache\n",
      );
      mkdirSync(join(dir, "apps", "docs", ".source"), { recursive: true });
      writeFileSync(
        join(dir, "apps", "docs", ".source", "llms.txt"),
        "# Fumadocs MDX typegen\n",
      );

      const result = await scan({ targetRepo: dir });
      const paths = result.files.map((f) => f.relPath).sort();
      expect(paths).toEqual(["llms.txt"]);
    } finally {
