...
Write one scenario with a `requiredSources`, an `expected`, and a `trap`. Score the same response twice and nothing moves; the only thing that can change a rerun is the agent's answer, not the scoring.
