...
The `injected` cell injects your `llms.txt` into the prompt. That is the content test: if it fails here, your `llms.txt` does not actually answer the question. The `web` cell injects nothing and requires the agent to invoke a web tool to reach the answer. That is the discovery test: if it fails here while `injected` passes, your content is fine but the agent did not reach it through the configured web path.
...
Point pickled at your `llms.txt` and write one question it is supposed to answer. Run it once with the file injected and once with the agent forced to reach it through web tools, and see which promise holds.
