...
Each `(agent × context)` cell runs `trials` times in a fresh workspace; the result is `k/n`. Builds run on edit-capable CLI agents (`claude-code`, `codex-cli`). The `failToPass` commands must fail on the untouched fixture and pass after the agent. See the [pickled.yml reference](/pickled-yml) for the full build contract.
