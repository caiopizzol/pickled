...
`threshold` is optional. Set it to make the run pass or fail on the overall score; omit it to see the score without a pass/fail verdict.
...
This produces one cell per `(interface × source × toolset)` combination. The [Matrix evaluation](/matrix-evaluation) page describes the full design and the `web` and `mcp` toolset shapes.

## Next steps

- Read about [matrix evaluation](/matrix-evaluation) to test discovery surfaces.
