...
## Testing checks offline (`examples` + `pickled test`)

Add sample answers to a scenario and score them with no model calls:
...
`pickled test` scores each `examples.pass` / `examples.fail` string against the scenario's `expected` and `traps` with the same matchers a real run uses, makes zero model calls, and exits non-zero if any example does not behave as declared. A `pass` example a check rejects means the check is too strict; a `pass` example a trap fires on means the trap is over-broad. Citation / `requiredSources` is not scored, since an example has no injected source.
